...
VoxelGrid::VoxelGrid(Eigen::Ref<const Eigen::Vector3d> origin, Eigen::Ref<const Eigen::Vector3i> dim,
...
VoxelGrid::VoxelGrid(Eigen::Ref<const Eigen::Vector3d> origin, Eigen::Ref<const Eigen::Vector3d> dim,
                     double vox_size, bool free)
...
voxel_data_type VoxelGrid::GetVoxelInt(Eigen::Ref<const Eigen::Vector3i> coord_int) const {
  int idx = CoordToIdx(coord_int);
  if (idx != -1) {
    return data_[idx];
  } else {
...
voxel_data_type VoxelGrid::GetVoxelInt(Eigen::Ref<const Eigen::Vector3d> coord_int) const {
