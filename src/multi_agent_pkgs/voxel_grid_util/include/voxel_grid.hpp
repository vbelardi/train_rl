...
public:
   ...
   */
  VoxelGrid(Eigen::Ref<const Eigen::Vector3d> origin, Eigen::Ref<const Eigen::Vector3i> dim, double vox_size,
            bool free);
   ...
   * @param free: if true, initialize the grid to free instead of unknown
   */
  VoxelGrid(Eigen::Ref<const Eigen::Vector3d> origin, Eigen::Ref<const Eigen::Vector3d> dim, double vox_size,
            bool free);
  ...
  // return -1 if the coordinate is outside the grid
  voxel_data_type GetVoxelInt(Eigen::Ref<const Eigen::Vector3i> coord_int) const;

  // get the voxel value from double coordinates in the grid frame (local)
  // return -1 if the coordinate is outside the grid
  voxel_data_type GetVoxelInt(Eigen::Ref<const Eigen::Vector3d> coord_int) const;
