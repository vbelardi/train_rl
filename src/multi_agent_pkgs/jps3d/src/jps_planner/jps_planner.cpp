...
template <int Dim>
bool JPSPlanner<Dim>::plan_occ(const Vecf<Dim> &start, const Vecf<Dim> &goal,
                               decimal_t eps, bool use_jps) {
  if (planner_verbose_) {
    std::cout << "Start: " << start.transpose() << std::endl;
    std::cout << "Goal:  " << goal.transpose() << std::endl;
  ...
  if (!map_util_->isFree(goal_int)) {
    ...
    if (!foundCandidate) {
      ...
      Vecf<Dim> newGoal = map_util_->intToFloat(goal_int);
      std::cout << "Original goal " << goal.transpose()
                << " is not free. Using new goal: " << newGoal.transpose()
                << std::endl;
    }
  }
